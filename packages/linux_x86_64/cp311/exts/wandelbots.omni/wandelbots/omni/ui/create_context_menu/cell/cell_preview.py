"""Viewport overlay that shows translucent collision-mesh previews for all
robots in the selected NOVA cell before the user clicks Confirm."""
...
_FRAME_NAME = "cell_spawn_preview"


class CellPreview:
    """Renders transparent collision meshes in the viewport for all robots in
    a NOVA cell at zero-joint-angle poses so the user can see the robots
    ...
    def __init__(self) -> None:
        ...
        self._active_cell_key: str | None = None
    ...
    def _ensure_scene(self) -> bool:
        ...
        if viewport is None:
            carb.log_warn("No active viewport for cell preview")
            return False
    ...
    def request_preview(
        self,
        cell: NOVACellData | None,
        instance: NOVAInstance | None,
        prim_path: str | None = None,
    ) -> None:
        cell_key = (
            f"{instance.display_name}/{cell.name}/{prim_path}"
            if (cell and instance)
            else None
        )
        if cell_key == self._active_cell_key:
            return
        ...
        if cell is None or instance is None:
            self.clear()
            return
        ...
        self._preview_task = run_coroutine(
            self._show_preview(cell, instance, prim_path)
        )
    ...
    async def _show_preview(
        ...
        self.clear()
        self._active_cell_key = f"{instance.display_name}/{cell.name}/{prim_path}"
        ...
        try:
            ...
            for controller in cell.controllers:
                for mg in controller.motion_groups:
                    ...
                    try:
                        ...
                        carb.log_warn(
                            f"Cell preview: could not fetch mounting for '{mg.name}': {exc}"
                        )
                    ...
                    try:
                        ...
                        carb.log_warn(
                            f"Cell preview: failed to fetch model data for '{model_name}': {exc}"
                        )
                        continue

                    dh_parameters = kinematic_model.dh_parameters
                    base_transform = self._compute_base_transform(
                        model_name, stage_units, mounting, prim_path
                    )
            ...
            carb.log_info(
                f"Cell preview: {len(self._meshes)} meshes for cell '{cell.name}'"
            )

        except asyncio.CancelledError:
            self.clear()
        except Exception as exc:
            carb.log_warn(f"Failed to create cell preview: {exc}")
            self.clear()
        finally:
            self._preview_task = None
            try:
    ...
    def destroy(self) -> None:
        ...
        if self._viewport is not None and self._scene_view is not None:
            try:
                ...
                carb.log_warn(f"Error removing cell preview scene view: {exc}")
        ...
        if prim_path:
            ...
            if stage:
                ...
                if prim.IsValid():
                    ...
                    flat = [world_mtx[col][row] for col in range(4) for row in range(4)]
                    location_matrix = sc.Matrix44(*flat)

        if mounting is not None:
            mounting_pose = list(mounting.position) + list(mounting.orientation)
            return location_matrix * nova_pose_to_scene_matrix(
                mounting_pose, stage_units
            )

        base_offset = MODEL_BASE_OFFSETS.get(model_name, 0.0)
        if base_offset != 0.0:
            offset_pose = [0, 0, base_offset * 1000.0, 0, 0, 0]
            return location_matrix * nova_pose_to_scene_matrix(offset_pose, stage_units)
