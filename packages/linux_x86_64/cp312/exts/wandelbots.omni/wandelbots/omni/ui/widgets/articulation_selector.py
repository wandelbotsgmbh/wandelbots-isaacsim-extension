...
class ArticulationSelector(ui.HStack):
    ...
    Always shows a ComboBox of available articulations. When a motion group is
    already connected the connected prim path is pre-selected; on an unreachable
    instance the ComboBox is disabled so the selection cannot be changed.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        with self:
            ...
            self._build_combo(connected_prim_path or initial_selection, read_only)
            ui.Spacer(width=10)
    ...
    def _build_combo(self, initial_selection: Optional[str], read_only: bool = False):
        ...
        combo = ui.ComboBox(
            ...
            *dropdown_items,
            ...
            tooltip_fn=lambda: build_tooltip(
                "Select the stage articulation to connect to this motion group."
            ),
        )
        combo.enabled = not read_only
