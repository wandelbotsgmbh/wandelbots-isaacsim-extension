...
class CollapsibleSection(ui.VStack):
        ...
        with self:
            with ui.ZStack(height=28):
                self._header_bg = ui.Rectangle(
                    style={
                        "background_color": NOVAColor.COLLAPSIBLE_SECTION_HEADER.color,
                        "border_radius": 2,
                    },
                )
    ...
    def _on_header_hover(self, hovered: bool):
        if not self._header_bg:
            return
        color = (
            NOVAColor.COLLAPSIBLE_SECTION_HEADER_HOVER.color
            if hovered
            else NOVAColor.COLLAPSIBLE_SECTION_HEADER.color
        )
