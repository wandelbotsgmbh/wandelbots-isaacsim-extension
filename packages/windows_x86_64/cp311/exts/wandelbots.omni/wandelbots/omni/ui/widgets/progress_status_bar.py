...
from wandelbots.omni.ui.wb_theme import FONT_SIZE_SM, PROGRESS_BAR_STYLE
...
class ProgressStatusBar:
    ...
    def __init__(self, name: str = "") -> None:
        ...
        self._percent_label: ui.Label | None = None
    ...
    def _update_percent(self, value: float) -> None:
        if self._percent_label:
            self._percent_label.text = f"{round(min(value, 1.0) * 100)}%"
            self._percent_label.visible = True
    ...
    def hide(self) -> None:
        if self._progress_bar:
            ...
            self._progress_bar.visible = False
        if self._percent_label:
            self._percent_label.text = ""
            self._percent_label.visible = False
        self.clear_hint()
    ...
    def restore_state(
        ...
        if progress_visible and self._progress_bar:
            ...
            self._progress_bar.visible = True
            self._update_percent(progress_value)
