...
def read_dependencies():
    toml_file_path =  os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(__file__)))), "config", "extension.toml")
    ...
    for dist in distributions():
        ...
        if dist.metadata["Name"] == dependency["package"]:
            ...
            if installed_version == dependency["version"]:
                carb.log_info(f"{package} ({version}) is up to date. Nothing to do here.")
                return True
